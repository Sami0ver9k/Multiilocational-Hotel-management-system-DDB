...
Begin
    ...
    if flag=1 then
    --get guest id
    gNum:=updateProcAtDhaka.getGuestId(fname,lname,tel);
    --Get new date to update
    ckIn:='&Enter_new_date';
    ckOut:='&Enter_new_date';
    --Change date
    updateProcAtDhaka.changeDate(gNum,ckIn,ckOut);
    else
    --Get room no
    rmNo:=updateProcAtDhaka.getGuestRoomNo(fname,lname,tel);
    --ChangeRooms
    rmNo2:=&Enter_new_roomNo;
    updateProcAtDhaka.changeRoom(rmNo,rmNo2);
    end if;
